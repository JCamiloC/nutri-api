  ...
  try {
    ...
    const result = await getPool().query(
      `UPDATE ingredients SET
        ...
        parte_analizada = COALESCE($10, parte_analizada),
        humedad = COALESCE($11, humedad),
        grasas = COALESCE($12, grasas),
        grasa_saturada = COALESCE($13, grasa_saturada),
        grasa_mono = COALESCE($14, grasa_mono),
        grasa_poli = COALESCE($15, grasa_poli),
        grasa_trans = COALESCE($16, grasa_trans),
        colesterol = COALESCE($17, colesterol),
        sodio = COALESCE($18, sodio),
        potasio = COALESCE($19, potasio),
        carbohidratos = COALESCE($20, carbohidratos),
        fibra = COALESCE($21, fibra),
        fibra_sol = COALESCE($22, fibra_sol),
        fibra_insol = COALESCE($23, fibra_insol),
        polialcoholes = COALESCE($24, polialcoholes),
        azucar = COALESCE($25, azucar),
        azucar_add = COALESCE($26, azucar_add),
        proteina = COALESCE($27, proteina),
        energia_kcal = COALESCE($28, energia_kcal),
        vitaminas = COALESCE($29::jsonb, vitaminas),
        alergenos = COALESCE($30::jsonb, alergenos),
        updated_at = now()
